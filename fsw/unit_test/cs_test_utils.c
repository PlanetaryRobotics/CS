...
CS_Res_EepromMemory_Table_Entry_t
    CS_DefaultEepromResTable[CS_MAX_NUM_EEPROM_TABLE_ENTRIES];
CS_Res_EepromMemory_Table_Entry_t
    CS_DefaultMemoryResTable[CS_MAX_NUM_MEMORY_TABLE_ENTRIES];
CS_Res_Tables_Table_Entry_t
    CS_DefaultTablesResTable[CS_MAX_NUM_TABLES_TABLE_ENTRIES];
...
void CS_Test_Setup(void) {
    ...
    memset(CS_DefaultEepromResTable, 0,
           sizeof(CS_Res_EepromMemory_Table_Entry_t) *
               CS_MAX_NUM_EEPROM_TABLE_ENTRIES);
    memset(CS_DefaultMemoryResTable, 0,
           sizeof(CS_Res_EepromMemory_Table_Entry_t) *
               CS_MAX_NUM_MEMORY_TABLE_ENTRIES);
    memset(
        CS_DefaultTablesResTable, 0,
        sizeof(CS_Res_Tables_Table_Entry_t) * CS_MAX_NUM_TABLES_TABLE_ENTRIES);
    memset(CS_DefaultAppResTable, 0,
           sizeof(CS_Res_App_Table_Entry_t) * CS_MAX_NUM_APP_TABLE_ENTRIES);
 ...
 *
 * Note: This code needs to be moved into the UT-Assert library.  We are
 * including it here for now because the next release of the UT-Assert library
...
Ut_CFE_PSP_MEMORY_ReturnCodeTable_t
    Ut_CFE_PSP_MEMORY_ReturnCodeTable[UT_CFE_PSP_MEMORY_MAX_INDEX];

void Ut_CFE_PSP_MEMORY_SetFunctionHook(uint32 Index, void *FunPtr) {
    if (Index == UT_CFE_PSP_MEMORY_GETKERNELTEXTSEGMENTINFO_INDEX) {
        Ut_CFE_PSP_MEMORY_HookTable.CFE_PSP_GetKernelTextSegmentInfo = FunPtr;
    } else if (Index == UT_CFE_PSP_MEMORY_GETCFETEXTSEGMENTINFO_INDEX) {
        Ut_CFE_PSP_MEMORY_HookTable.CFE_PSP_GetCFETextSegmentInfo = FunPtr;
    } else {
...
void Ut_CFE_PSP_MEMORY_SetReturnCode(uint32 Index, int32 RtnVal,
                                     uint32 CallCnt) {
    if (Index < UT_CFE_PSP_MEMORY_MAX_INDEX) {
        ...
        printf("Unsupported PSP_MEMORY Index In SetReturnCode Call %lu\n",
               Index);
        UtAssert_True(FALSE,
                      "Unsupported PSP_MEMORY Index In SetReturnCode Call");
    }
}
...
boolean Ut_CFE_PSP_MEMORY_UseReturnCode(uint32 Index) {
    if (Ut_CFE_PSP_MEMORY_ReturnCodeTable[Index].Count > 0) {
        ...
        if (Ut_CFE_PSP_MEMORY_ReturnCodeTable[Index].Count == 0) return (TRUE);
    } else if (Ut_CFE_PSP_MEMORY_ReturnCodeTable[Index]
...
void Ut_CFE_PSP_MEMORY_ContinueReturnCodeAfterCountZero(uint32 Index) {
    Ut_CFE_PSP_MEMORY_ReturnCodeTable[Index].ContinueReturnCodeAfterCountZero =
        TRUE;
}
...
int32 CFE_PSP_GetKernelTextSegmentInfo(cpuaddr *PtrToKernelSegment,
    ...
    if (Ut_CFE_PSP_MEMORY_UseReturnCode(
            UT_CFE_PSP_MEMORY_GETKERNELTEXTSEGMENTINFO_INDEX))
        return Ut_CFE_PSP_MEMORY_ReturnCodeTable
            [UT_CFE_PSP_MEMORY_GETKERNELTEXTSEGMENTINFO_INDEX]
                .Value;

    /* Check for Function Hook */
    if (Ut_CFE_PSP_MEMORY_HookTable.CFE_PSP_GetKernelTextSegmentInfo)
        return (Ut_CFE_PSP_MEMORY_HookTable.CFE_PSP_GetKernelTextSegmentInfo(
            PtrToKernelSegment, SizeOfKernelSegment));
...
int32 CFE_PSP_GetCFETextSegmentInfo(cpuaddr *PtrToCFESegment,
    ...
    if (Ut_CFE_PSP_MEMORY_HookTable.CFE_PSP_GetCFETextSegmentInfo)
        return (Ut_CFE_PSP_MEMORY_HookTable.CFE_PSP_GetCFETextSegmentInfo(
            PtrToCFESegment, SizeOfCFESegment));
