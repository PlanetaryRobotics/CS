...
** Add UtAssert fail where error messages, add return, add NotifyByMessage, add
*ContinueReturnCodeAfterCountZero
...
*c:/MKSDATA/MKS-REPOSITORY/FSW-TOOLS-REPOSITORY/ut-assert/src/project.pj
**
*/
...
void Ut_CFE_TBL_Reset(void) {
    ...
    Ut_CFE_TBL_SetFunctionHook(UT_CFE_TBL_REGISTER_INDEX,
                               (void *)&Ut_CFE_TBL_RegisterHook);
    Ut_CFE_TBL_SetFunctionHook(UT_CFE_TBL_LOAD_INDEX,
                               (void *)&Ut_CFE_TBL_LoadHook);
    Ut_CFE_TBL_SetFunctionHook(UT_CFE_TBL_GETADDRESS_INDEX,
                               (void *)&Ut_CFE_TBL_GetAddressHook);

    Ut_CFE_TBL_ClearTables();
}

void Ut_CFE_TBL_SetFunctionHook(uint32 Index, void *FunPtr) {
    if (Index == UT_CFE_TBL_REGISTER_INDEX) {
        Ut_CFE_TBL_HookTable.CFE_TBL_Register = FunPtr;
    } else if (Index == UT_CFE_TBL_SHARE_INDEX) {
        Ut_CFE_TBL_HookTable.CFE_TBL_Share = FunPtr;
    } else if (Index == UT_CFE_TBL_LOAD_INDEX) {
        Ut_CFE_TBL_HookTable.CFE_TBL_Load = FunPtr;
    } else if (Index == UT_CFE_TBL_MANAGE_INDEX) {
        Ut_CFE_TBL_HookTable.CFE_TBL_Manage = FunPtr;
    } else if (Index == UT_CFE_TBL_GETADDRESS_INDEX) {
        Ut_CFE_TBL_HookTable.CFE_TBL_GetAddress = FunPtr;
    } else if (Index == UT_CFE_TBL_GETADDRESSES_INDEX) {
        Ut_CFE_TBL_HookTable.CFE_TBL_GetAddresses = FunPtr;
    } else if (Index == UT_CFE_TBL_GETINFO_INDEX) {
        Ut_CFE_TBL_HookTable.CFE_TBL_GetInfo = FunPtr;
    } else if (Index == UT_CFE_TBL_NOTIFYBYMESSAGE_INDEX) {
        Ut_CFE_TBL_HookTable.CFE_TBL_NotifyByMessage = FunPtr;
    } else if (Index == UT_CFE_TBL_GETSTATUS_INDEX) {
        Ut_CFE_TBL_HookTable.CFE_TBL_GetStatus = FunPtr;
    } else {
...
boolean Ut_CFE_TBL_UseReturnCode(uint32 Index) {
    if (Ut_CFE_TBL_ReturnCodeTable[Index].Count > 0) {
        ...
        if (Ut_CFE_TBL_ReturnCodeTable[Index].Count == 0) return (TRUE);
    } else if (Ut_CFE_TBL_ReturnCodeTable[Index]
                   .ContinueReturnCodeAfterCountZero == TRUE) {
        return (TRUE);
    }
...
int32 CFE_TBL_Register(CFE_TBL_Handle_t *TblHandlePtr, const char *Name,
    ...
    if (Ut_CFE_TBL_HookTable.CFE_TBL_Register)
        return (Ut_CFE_TBL_HookTable.CFE_TBL_Register(
            TblHandlePtr, Name, Size, TblOptionFlags, TblValidationFuncPtr));
...
int32 CFE_TBL_Load(CFE_TBL_Handle_t TblHandle, CFE_TBL_SrcEnum_t SrcType,
    ...
    if (Ut_CFE_TBL_HookTable.CFE_TBL_Load)
        return (
            Ut_CFE_TBL_HookTable.CFE_TBL_Load(TblHandle, SrcType, SrcDataPtr));
...
int32 CFE_TBL_GetAddresses(void **TblPtrs[], uint16 NumTables,
    ...
    if (Ut_CFE_TBL_HookTable.CFE_TBL_GetAddresses)
        return (Ut_CFE_TBL_HookTable.CFE_TBL_GetAddresses(TblPtrs, NumTables,
                                                          TblHandles));
...
int32 CFE_TBL_ReleaseAddresses(uint16 NumTables,
    ...
    if (Ut_CFE_TBL_UseReturnCode(UT_CFE_TBL_RELEASEADDRESSES_INDEX))
        return Ut_CFE_TBL_ReturnCodeTable[UT_CFE_TBL_RELEASEADDRESSES_INDEX]
            .Value;

    /* Check for Function Hook */
    if (Ut_CFE_TBL_HookTable.CFE_TBL_ReleaseAddresses)
        return (Ut_CFE_TBL_HookTable.CFE_TBL_ReleaseAddresses(NumTables,
                                                              TblHandles));
...
int32 CFE_TBL_NotifyByMessage(CFE_TBL_Handle_t TblHandle, uint32 MsgId,
                              uint16 CommandCode, uint32 Parameter) {
    /* Check for specified return */
    if (Ut_CFE_TBL_UseReturnCode(UT_CFE_TBL_NOTIFYBYMESSAGE_INDEX))
        return Ut_CFE_TBL_ReturnCodeTable[UT_CFE_TBL_NOTIFYBYMESSAGE_INDEX]
            .Value;

    /* Check for Function Hook */
    if (Ut_CFE_TBL_HookTable.CFE_TBL_NotifyByMessage)
        return (Ut_CFE_TBL_HookTable.CFE_TBL_NotifyByMessage(
            TblHandle, MsgId, CommandCode, Parameter));
