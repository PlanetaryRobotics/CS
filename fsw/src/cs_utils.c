...
bool CS_GetTableDefTblEntryByName(CS_Def_Tables_Table_Entry_t** EntryPtr,
    ...
    for (Loop = 0; Loop < MaxEntries; Loop++) {
        ...
        if (strncmp(DefinitionEntry->Name, Name, CFE_TBL_MAX_FULL_NAME_LEN) ==
                0 &&
...
bool CS_BackgroundCfeCore(void) {
    ...
    if (CS_AppData.CfeCoreCSState == CS_STATE_ENABLED) {
        ...
        if (ResultsEntry->State == CS_STATE_ENABLED) {
            /* If we complete an entry's checksum, this function will update it
             * for us */
            Status = CS_ComputeEepromMemory(ResultsEntry, &ComputedCSValue,
                                            &DoneWithEntry);

            /* We need to avoid the case of finishing a table, moving on to the
             next one and computing an entry in that table, since it could put
             us above the maximum bytes per cycle */
            DoneWithCycle = true;
        ...
        if (ResultsEntry->State == CS_STATE_ENABLED) {
            ...
            if (Status == CS_ERROR) {
                ...
                CFE_EVS_SendEvent(CS_OS_MISCOMPARE_ERR_EID,
                                  CFE_EVS_EventType_ERROR,
                                  "Checksum Failure: OS code segment, "
    ...
    if (CS_AppData.EepromCSState == CS_STATE_ENABLED) {
        if (CS_FindEnabledEepromEntry(&CurrEntry) == true) {
            ...
            if (Status == CS_ERROR) {
                ...
                CFE_EVS_SendEvent(
                    CS_EEPROM_MISCOMPARE_ERR_EID, CFE_EVS_EventType_ERROR,
                    "Checksum Failure: Entry %d in Eeprom Table, Expected: "
                    "0x%08X, Calculated: 0x%08X",
            ...
            if (CS_AppData.CurrentEntryInTable >=
                CS_MAX_NUM_EEPROM_TABLE_ENTRIES) {
                /* Since we are done CS'ing the entire Eeprom table, update the
                 baseline number for telemetry */
                EntireEepromCS = 0;
                for (Loop = 0; Loop < CS_MAX_NUM_EEPROM_TABLE_ENTRIES; Loop++) {
                    EntireEepromCS +=
                        CS_AppData.ResEepromTblPtr[Loop].ComparisonValue;
                }
...
bool CS_BackgroundMemory(void) {
    ...
    if (CS_AppData.MemoryCSState == CS_STATE_ENABLED) {
        /* If we complete an entry's checksum, this function will update it for
         * us */
        ...
        if (CS_FindEnabledMemoryEntry(&CurrEntry) == true) {
            ...
            if (Status == CS_ERROR) {
                ...
                CFE_EVS_SendEvent(
                    CS_MEMORY_MISCOMPARE_ERR_EID, CFE_EVS_EventType_ERROR,
                    "Checksum Failure: Entry %d in Memory Table, Expected: "
                    "0x%08X, Calculated: 0x%08X",
                    CurrEntry, (unsigned int)(ResultsEntry->ComparisonValue),
                    (unsigned int)ComputedCSValue);
        ...
        } else /* since there aren't any enabled entries in this table, go to
                  the next one */
        {
            CS_GoToNextTable();
        }
...
bool CS_BackgroundTables(void) {
    ...
    if (CS_AppData.TablesCSState == CS_STATE_ENABLED) {
        ...
        if (CS_FindEnabledTablesEntry(&CurrEntry) == true) {
            ...
            Status = CS_ComputeTables(TablesResultsEntry, &ComputedCSValue,
                                      &DoneWithEntry);
            ...
            if (Status == CS_ERROR) {
                ...
                CFE_EVS_SendEvent(
                    CS_TABLES_MISCOMPARE_ERR_EID, CFE_EVS_EventType_ERROR,
                    "Checksum Failure: Table %s, Expected: 0x%08X, Calculated: "
                    "0x%08X",
            ...
            if (Status == CS_ERR_NOT_FOUND) {
                CFE_EVS_SendEvent(CS_COMPUTE_TABLES_NOT_FOUND_ERR_EID,
                                  CFE_EVS_EventType_ERROR,
                                  "Tables table computing: Table %s could not "
                                  "be found, skipping",
                                  TablesResultsEntry->Name);
...
bool CS_BackgroundApp(void) {
    ...
    if (CS_AppData.AppCSState == CS_STATE_ENABLED) {
        if (CS_FindEnabledAppEntry(&CurrEntry) == true) {
            ...
            /* If we complete an entry's checksum, this function will update it
             * for us */
            Status = CS_ComputeApp(AppResultsEntry, &ComputedCSValue,
                                   &DoneWithEntry);
            ...
            if (Status == CS_ERROR) {
                ...
                CFE_EVS_SendEvent(
                    CS_APP_MISCOMPARE_ERR_EID, CFE_EVS_EventType_ERROR,
                    "Checksum Failure: Application %s, Expected: 0x%08X, "
                    "Calculated: 0x%08X",
            ...
            if (Status == CS_ERR_NOT_FOUND) {
                CFE_EVS_SendEvent(
                    CS_COMPUTE_APP_NOT_FOUND_ERR_EID, CFE_EVS_EventType_ERROR,
