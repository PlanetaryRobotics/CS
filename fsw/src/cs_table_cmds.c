        ...
        if (CS_AppData.RecomputeInProgress == false &&
            ...
            if (CS_GetTableResTblEntryByName(&ResultsEntry, CmdPtr->Name)) {
                ...
                Status = CFE_ES_CreateChildTask(
                    &ChildTaskID, CS_RECOMP_TABLES_TASK_NAME,
                    CS_RecomputeTablesChildTask, NULL,
                ...
                } else /* child task creation failed */
                {
                    CFE_EVS_SendEvent(
                        ...
                        CFE_EVS_EventType_ERROR,
                        "Recompute baseline of table %s failed, "
            ...
            CFE_EVS_SendEvent(CS_RECOMPUTE_TABLES_CHDTASK_ERR_EID,
                              CFE_EVS_EventType_ERROR,
                              "Tables recompute baseline for table %s failed: "
                              "child task in use",
                              CmdPtr->Name);
            CS_AppData.CmdErrCounter++;
        }
...
void CS_DisableNameTablesCmd(CFE_SB_MsgPtr_t MessagePtr) {
    ...
    if (CS_VerifyCmdLength(MessagePtr, ExpectedLength)) {
        ...
        if (CS_GetTableResTblEntryByName(&ResultsEntry, CmdPtr->Name)) {
            ...
            CFE_EVS_SendEvent(
                CS_DISABLE_TABLES_NAME_INF_EID, CFE_EVS_EventType_INFORMATION,
                "Checksumming of table %s is Disabled", CmdPtr->Name);
            ...
            CFE_EVS_SendEvent(
                CS_DISABLE_TABLES_UNKNOWN_NAME_ERR_EID, CFE_EVS_EventType_ERROR,
...
void CS_EnableNameTablesCmd(CFE_SB_MsgPtr_t MessagePtr) {
    ...
    if (CS_VerifyCmdLength(MessagePtr, ExpectedLength)) {
        ...
        if (CS_GetTableResTblEntryByName(&ResultsEntry, CmdPtr->Name)) {
            ...
            CFE_EVS_SendEvent(
                CS_ENABLE_TABLES_NAME_INF_EID, CFE_EVS_EventType_INFORMATION,
                "Checksumming of table %s is Enabled", CmdPtr->Name);
            ...
            CFE_EVS_SendEvent(
                CS_ENABLE_TABLES_UNKNOWN_NAME_ERR_EID, CFE_EVS_EventType_ERROR,
