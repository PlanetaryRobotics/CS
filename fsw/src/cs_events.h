...
**  \par Cause:
**
**  This event message is issued when a new definition table has finished being
*processed
**  and there are no valid entries in that table
**
**  The \c table field specifies whether the table was the Eeprom or Memory
*table
*/
 ...
 **  \par Cause:
 **
 **  This event message is issued when CFE_SB_CreatePipe fails for the command
 *pipe
 **
...
**  \par Cause:
**
**  This event message is issued when CFE_SB_Subscribe failed to subscribe to
*the
**  housekeeping MID.
**
 ...
 *Memory is disabled' </tt>
 ** \event <tt> 'Table initialization failed for Memory: 0x\%08X, checksumming
 *Memory is disabled' </tt>
 **
 **  \par Type: ERROR
 **
...
/** \brief <tt> 'Table initialization failed for Tables: 0x\%08X, checksumming
 *Tables is disabled' </tt>
 ** \event <tt> 'Table initialization failed for Tables: 0x\%08X, checksumming
 *Tables is disabled' </tt>
 **
 **  \par Type: ERROR
 **
...
/** \brief <tt> 'Table initialization failed for Apps: 0x\%08X, checksumming
 *Apps is disabled' </tt>
 ** \event <tt> 'Table initialization failed for Apps: 0x\%08X, checksumming
 *Apps is disabled' </tt>
 **
 **  \par Type: ERROR
 **
...
/** \brief <tt> 'CS Tables: Could not release addresss for table \%s, returned:
 *0x\%08X' </tt>
 ** \event <tt> 'CS Tables: Could not release addresss for table \%s, returned:
 *0x\%08X' </tt>
 **
 **  \par Type: ERROR
 **
 **  \par Cause:
 **
 **  This event message is issued when Table Services can't release the address
 *for the table
 **  being checksummed.
 **
 **  The \c table field specifies the name of the table whose address can't be
 *released. <br>
 **  The \c returned field specifies the return code of CFE_TBL_ReleaseAddress.
 */
#define CS_COMPUTE_TABLES_RELEASE_ERR_EID 120

/** \brief <tt> 'CS Tables: Problem Getting table \%s info Share: 0x\%08X,
 *GetInfo: 0x\%08X, GetAddress: 0x\%08X' </tt>
 ** \event <tt> 'CS Tables: Problem Getting table \%s info Share: 0x\%08X,
 *GetInfo: 0x\%08X, GetAddress: 0x\%08X' </tt>
 **
 **  \par Type: ERROR
 **
 **  \par Cause:
 **
 **  This event message is issued when there was a problem getting the table
 *information to checksum.
 **
...
/** \brief <tt> 'CS Apps: Problems getting app \%s info, GetAppID: 0x\%08X,
 *GetAppInfo: 0x\%08X, AddressValid: 0x\%08X' </tt>
 ** \event <tt> 'CS Apps: Problems getting app \%s info, GetAppID: 0x\%08X,
 *GetAppInfo: 0x\%08X, AddressValid: 0x\%08X' </tt>
 **
 **  \par Type: ERROR
 **
 **  \par Cause:
 **
 **  This event message is issued when there was a problem getting the app
 *information to checksum.
 **
 **  The \c app field specifies the name of the app to be checksummed. <br>
 **  The \c GetAppID field specifies the return code of CFE_ES_GetAppIDByName.
 *<br>
 **  The \c GetAppInfo field specifies the return code of CFE_ES_GetAppInfo.
 *<br>
 **  The \c AddressValid field specifies whether or not the addresses obtained
 *from CFE_ES_GetAppInfo are valid.
 */

#define CS_COMPUTE_APP_ERR_EID 122
/*************************************************************************************************/

/** \brief <tt> 'Table update failed for Eeprom: 0x\%08X, checksumming Eeprom is
 *disabled' </tt>
 ** \event <tt> 'Table update failed for Eeprom: 0x\%08X, checksumming Eeprom is
 *disabled' </tt>
 **
 **  \par Type: ERROR
 **
...
/** \brief <tt> 'Table update failed for Memory: 0x\%08X, checksumming Memory is
 *disabled' </tt>
 ** \event <tt> 'Table update failed for Memory: 0x\%08X, checksumming Memory is
 *disabled' </tt>
 **
 **  \par Type: ERROR
 **
...
/** \brief <tt> 'Table update failed for Tables: 0x\%08X, checksumming Tables is
 *disabled' </tt>
 ** \event <tt> 'Table update failed for Tables: 0x\%08X, checksumming Tables is
 *disabled' </tt>
 **
 **  \par Type: ERROR
 **
...
/** \brief <tt> 'Table update failed for Apps: 0x\%08X, checksumming Apps is
 *disabled' </tt>
 ** \event <tt> 'Table update failed for Apps: 0x\%08X, checksumming Apps is
 *disabled' </tt>
 **
 **  \par Type: ERROR
 **
 ...
 **  \par Cause:
 **
 **  This event message is issued when CS is running on a platform that does not
 *support
...
/** \brief <tt> 'CS cannot get a valid address for \%s, due to the platform'
 *</tt>
 ** \event <tt> 'CS cannot get a valid address for \%s, due to the platform'
 *</tt>
 **
 **  \par Type: DEBUG
 **
 ...
 **  This event message is issued when CS is running on a platform that does not
 *support
 **  getting the address information of the applications, such as OS X and
 *Linux.
 **
 */
#define CS_COMPUTE_APP_PLATFORM_DBG_EID 128

/** \brief <tt> 'CS unable to update tables definition table for entry \%s'
 *</tt>
 ** \event <tt> 'CS unable to update tables definition table for entry \%s'
 *</tt>
 **
 **  \par Type: DEBUG
 **
 **  \par Cause:
 **
 **  This event message is issued when CS successfully enables an entry
 *(specified
 ...
 **  \par Cause:
 **
 **  This event message is issued when CS successfully disables an entry
 *(specified
 **  by name) in the Tables results table but is unable to find the same entry
 **  in the definition table (or the entry is marked as #CS_STATE_EMPTY).
 **
...
/** \brief <tt> 'CS unable to update memory definition table for entry \%d,
 *State: \%d' </tt>
 ** \event <tt> 'CS unable to update memory definition table for entry \%d,
 *State: \%d' </tt>
 **
 **  \par Type: DEBUG
 **
 ...
 **  This event message is issued when CS successfully disables an entry
 *(specified
 **  by name) in the Memory results table but identifies the corresponding entry
 *in
...
/** \brief <tt> 'CS unable to update Eeprom definition table for entry \%d,
 *State: \%d' </tt>
 ** \event <tt> 'CS unable to update Eeprom definition table for entry \%d,
 *State: \%d' </tt>
 **
 **  \par Type: DEBUG
 **
 ...
 **  This event message is issued when CS successfully disables an entry
 *(specified
 **  by name) in the Eeprom results table but identifies the corresponding entry
 *in
 **  the definitions table to be set to #CS_STATE_EMPTY.
 **
 */
...
/** \brief <tt> 'CS Tables Table Validate: Duplicate Name (\%s) found at entries
 *\%d and \%d' </tt>
 ** \event <tt> 'CS Tables Table Validate: Duplicate Name (\%s) found at entries
 *\%d and \%d' </tt>
 **
 **  \par Type: ERROR
 **
 **  \par Cause:
 **
 **  This event message is issued when CS validation for the Tables definition
 *table finds more
 **  than one entry with the same table name.  Only one entry per table is
 *allowed.  This event
...
/** \brief <tt> 'CS Tables Table Validate: Illegal State (0x\%04X) with empty
 *name at entry \%d' </tt>
 ** \event <tt> 'CS Tables Table Validate: Illegal State (0x\%04X) with empty
 *name at entry \%d' </tt>
 **
 **  \par Type: ERROR
 **
 ...
 **
 **  This event message is issued when CS validation for the Tables definition
 *table finds an entry
 **  that contains a zero-length name field with a state field that is not
 *marked as #CS_STATE_EMPTY.  This
 **  event is only issued if it is the first error found during validation.
 **
 */
#define CS_VAL_TABLES_DEF_TBL_ZERO_NAME_ERR_EID 138

/** \brief <tt> 'CS Tables Table verification results: good = \%d, bad = \%d,
 *unused = \%d' </tt>
 ** \event <tt> 'CS Tables Table verification results: good = \%d, bad = \%d,
 *unused = \%d' </tt>
 **
 **  \par Type: INFORMATION
 **
 ...
 *table.  This message
 **  reports the number of successful (#CS_STATE_ENABLED or #CS_STATE_DISABLED)
 *entries, the number of
 **  bad entries (due to invalid state definitions or duplicate names), and the
 *number of entries
...
/** \brief <tt> 'CS Apps Table Validate: Duplicate Name (\%s) found at entries
 *\%d and \%d' </tt>
 ** \event <tt> 'CS Apps Table Validate: Duplicate Name (\%s) found at entries
 *\%d and \%d' </tt>
 **
 **  \par Type: ERROR
 **
 **  \par Cause:
 **
 **  This event message is issued when CS validation for the Apps definition
 *table finds more
 **  than one entry with the same table name.  Only one entry per table is
 *allowed.  This event
 **  is only issued if it is the first error found during validation.
...
/** \brief <tt> 'CS Apps Table Validate: Illegal State (0x\%04X) with empty name
 *at entry \%d' </tt>
 ** \event <tt> 'CS Apps Table Validate: Illegal State (0x\%04X) with empty name
 *at entry \%d' </tt>
 **
 **  \par Type: ERROR
 **
...
/** \brief <tt> 'CS Apps Table verification results: good = \%d, bad = \%d,
 *unused = \%d' </tt>
 ** \event <tt> 'CS Apps Table verification results: good = \%d, bad = \%d,
 *unused = \%d' </tt>
 **
 **  \par Type: INFORMATION
 **
 **  \par Cause:
 **
 **  This event message when CS completes validation of the Apps definition
 *table.  This message
 **  reports the number of successful (#CS_STATE_ENABLED or #CS_STATE_DISABLED)
 *entries, the number of
 **  bad entries (due to invalid state definitions or duplicate names), and the
...
/** \brief <tt> 'CS Memory Table verification results: good = \%d, bad = \%d,
 *unused = \%d' </tt>
 ** \event <tt> 'CS Memory Table verification results: good = \%d, bad = \%d,
 *unused = \%d' </tt>
 **
 **  \par Type: INFORMATION
 **
 **  \par Cause:
 **
 **  This event message when CS completes validation of the Memory definition
 *table.  This message
 **  reports the number of successful (#CS_STATE_ENABLED or #CS_STATE_DISABLED)
 *entries, the number of
 **  bad entries (due to invalid state definitions or bad range), and the number
 *of entries
 **  marked as #CS_STATE_EMPTY.
 **
 */
...
/** \brief <tt> 'CS Eeprom Table verification results: good = \%d, bad = \%d,
 *unused = \%d' </tt>
 ** \event <tt> 'CS Eeprom Table verification results: good = \%d, bad = \%d,
 *unused = \%d' </tt>
 **
 **  \par Type: INFORMATION
 **
 **  \par Cause:
 **
 **  This event message when CS completes validation of the Eeprom definition
 *table.  This message
 **  reports the number of successful (#CS_STATE_ENABLED or #CS_STATE_DISABLED)
 *entries, the number of
 **  bad entries (due to invalid state definitions or bad range), and the number
