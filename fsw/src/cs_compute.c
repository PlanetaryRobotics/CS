...
int32 CS_ComputeEepromMemory(CS_Res_EepromMemory_Table_Entry_t *ResultsEntry,
                             uint32 *ComputedCSValue, bool *DoneWithEntry) {
    ...
    NumBytesRemainingCycles =
        ResultsEntry->NumBytesToChecksum - OffsetIntoCurrEntry;
    ...
    NewChecksumValue = CFE_ES_CalculateCRC(
        (void *)(FirstAddrThisCycle), NumBytesThisCycle,
...
int32 CS_ComputeTables(CS_Res_Tables_Table_Entry_t *ResultsEntry,
    ...
    *DoneWithEntry = false;
    ...
    if (Result == CFE_SUCCESS) {
        ResultGetAddress =
            CFE_TBL_GetAddress((void *)&LocalAddress, LocalTblHandle);
        Result = ResultGetAddress;
    }

    /* if the table was never loaded, release the address to prevent the table
       from being
    ...
    if (Result == CFE_TBL_ERR_UNREGISTERED) {
        ...
        if (ResultShare == CFE_SUCCESS) {
            ...
            /* if the table was never loaded, release the address to prevent the
             table from being locked by CS, which would prevent the owner app
             from updating it*/
            if (ResultGetAddress == CFE_TBL_ERR_NEVER_LOADED) {
                CFE_TBL_ReleaseAddress(LocalTblHandle);
            }
    ...
    if (Result == CFE_SUCCESS || Result == CFE_TBL_INFO_UPDATED) {
        ...
        NumBytesThisCycle =
            ((CS_AppData.MaxBytesPerCycle < NumBytesRemainingCycles)
                 ? CS_AppData.MaxBytesPerCycle
                 : NumBytesRemainingCycles);
        ...
        if (NumBytesRemainingCycles <= 0) {
                ...
                *DoneWithEntry = true;
                ...
                if (ResultsEntry->ComputedYet == true) {
                    /* This is NOT the first time through this Entry.
                       We have already computed a CS value for this Entry */
                    if (NewChecksumValue != ResultsEntry->ComparisonValue) {
                        /* If the just-computed value differ from the saved
                         * value */
                        Status = CS_ERROR;
                    } else {
                        /* The checksum passes the test. */
            ...
            /* We have  not finished this Entry.  Will try to finish during next
             * wakeup */
            ResultsEntry->ByteOffset += NumBytesThisCycle;
            ResultsEntry->TempChecksumValue = NewChecksumValue;
            *ComputedCSValue                = NewChecksumValue;
        ...
        /* We are done with the table for this cycle, so we need to release the
         * address */
        ...
        CFE_EVS_SendEvent(CS_COMPUTE_TABLES_ERR_EID, CFE_EVS_EventType_ERROR,
                          "CS Tables: Problem Getting table %s info Share: "
                          "0x%08X, GetInfo: 0x%08X, GetAddress: 0x%08X",
                          ResultsEntry->Name, (unsigned int)ResultShare,
...
int32 CS_ComputeApp(CS_Res_App_Table_Entry_t *ResultsEntry,
    ...
    *DoneWithEntry = false;
    ...
    if (Result == CFE_SUCCESS) {
        /* We got a valid AppID and good App info, so check the for valid
         * addresses */
    ...
    if (Result == CFE_SUCCESS) {
        /* We got valid AppID, good info, and valid addresses, so run the
         * checksum */
        ...
        /* Something failed -- either invalid AppID, bad App info, or invalid
         * addresses, so notify ground */
        CFE_EVS_SendEvent(CS_COMPUTE_APP_ERR_EID, CFE_EVS_EventType_ERROR,
                          "CS Apps: Problems getting app %s info, GetAppID: "
                          "0x%08X, GetAppInfo: 0x%08X, AddressValid: %d",
                          ResultsEntry->Name, (unsigned int)ResultGetAppID,
...
void CS_RecomputeEepromMemoryChildTask(void) {
    ...
    strncpy(TableType, "Undef Tbl",
            CS_TABLETYPE_NAME_SIZE); /* Initialize table type string */
    ...
    if (Status == CFE_SUCCESS) {
        ...
        /* Update the definition table entry as well.  We need to determine
           which memory type is being updated as well as which entry in the
        ...
        if ((Table != CS_OSCORE) && (Table != CS_CFECORE)) {
            ...
            if (EntryID < MaxDefEntries) {
                /* This assumes that the definition table entries are in the
                   same order as the results table entries, which should be a
                   safe assumption. */
                if ((ResultsEntry->StartAddress ==
                     DefTblPtr[EntryID].StartAddress) &&
        ...
        while (!DoneWithEntry) {
            CS_ComputeEepromMemory(ResultsEntry, &NewChecksumValue,
                                   &DoneWithEntry);
        ...
        CFE_EVS_SendEvent(
            ...
            "%s entry %d recompute finished. New baseline is 0X%08X", TableType,
            EntryID, (unsigned int)NewChecksumValue);
    } /* end if child task register */
    else {
        /* Can't send event or write to syslog because this task isn't
         * registered with the cFE. */
        OS_printf(
            "Recompute for Eeprom or Memory Child Task Registration failed!\n");
    }
...
void CS_RecomputeAppChildTask(void) {
    ...
    if (Status == CFE_SUCCESS) {
        ...
        while ((!DefEntryFound) && (DefEntryID < MaxDefEntries)) {
            if ((strncmp(ResultsEntry->Name, DefTblPtr[DefEntryID].Name,
                         OS_MAX_API_NAME) == 0) &&
        ...
        while (!DoneWithEntry) {
            Status =
                CS_ComputeApp(ResultsEntry, &NewChecksumValue, &DoneWithEntry);
        ...
        if (Status == CS_ERR_NOT_FOUND) {
            ...
            /* send event message */
            CFE_EVS_SendEvent(
                CS_RECOMPUTE_FINISH_APP_INF_EID, CFE_EVS_EventType_INFORMATION,
                "App %s recompute finished. New baseline is 0x%08X",
                ResultsEntry->Name, (unsigned int)NewChecksumValue);
        }
...
void CS_RecomputeTablesChildTask(void) {
    ...
    if (Status == CFE_SUCCESS) {
        ...
        while (!DoneWithEntry) {
            Status = CS_ComputeTables(ResultsEntry, &NewChecksumValue,
                                      &DoneWithEntry);
        ...
        if (Status == CS_ERR_NOT_FOUND) {
            CFE_EVS_SendEvent(
                CS_RECOMPUTE_ERROR_TABLES_ERR_EID, CFE_EVS_EventType_ERROR,
...
void CS_OneShotChildTask(void) {
    ...
    if (Status == CFE_SUCCESS) {
        ...
        while (NumBytesRemainingCycles > 0) {
            ...
            NewChecksumValue = CFE_ES_CalculateCRC(
                (void *)(FirstAddrThisCycle), NumBytesThisCycle,
                NewChecksumValue, CS_DEFAULT_ALGORITHM);
        ...
        CFE_EVS_SendEvent(CS_ONESHOT_FINISHED_INF_EID,
                          CFE_EVS_EventType_INFORMATION,
                          "OneShot checksum on Address: 0x%08X, size %d "
                          "completed. Checksum =  0x%08X",
