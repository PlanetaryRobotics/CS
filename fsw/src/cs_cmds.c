...
void CS_NoopCmd(CFE_SB_MsgPtr_t MessagePtr) {
    ...
    if (CS_VerifyCmdLength(MessagePtr, ExpectedLength)) {
        ...
        CFE_EVS_SendEvent(CS_NOOP_INF_EID, CFE_EVS_EventType_INFORMATION,
                          ...
                          CS_MAJOR_VERSION, CS_MINOR_VERSION, CS_REVISION,
                          CS_MISSION_REV);
    }
    return;
} /* End of CS_NoopCmd () */
    ...
    if (ExpectedLength != ActualLength) {
        ...
        if (CS_AppData.ChecksumState == CS_STATE_ENABLED) {
            ...
            while ((DoneWithCycle != true) && (EndOfList != true)) {
                ...
                /* Call the appropriate background function based on the current
                   table value.  The value is updated inside each function */
                switch (CS_AppData.CurrentCSTable) {
                    ...
                    case (CS_APP_TABLE):
                        ...
                        /* default case in case CS_AppData.CurrentCSTable is
                         * some random bad value */
                    default:
...
void CS_DisableAllCSCmd(CFE_SB_MsgPtr_t MessagePtr) {
    ...
    if (CS_VerifyCmdLength(MessagePtr, ExpectedLength)) {
        ...
        CFE_EVS_SendEvent(CS_DISABLE_ALL_INF_EID, CFE_EVS_EventType_INFORMATION,
...
void CS_EnableAllCSCmd(CFE_SB_MsgPtr_t MessagePtr) {
    ...
    if (CS_VerifyCmdLength(MessagePtr, ExpectedLength)) {
        ...
        CFE_EVS_SendEvent(CS_ENABLE_ALL_INF_EID, CFE_EVS_EventType_INFORMATION,
...
void CS_ReportBaselineCfeCoreCmd(CFE_SB_MsgPtr_t MessagePtr) {
    ...
    if (CS_VerifyCmdLength(MessagePtr, ExpectedLength)) {
        if (CS_AppData.CfeCoreCodeSeg.ComputedYet == true) {
            CFE_EVS_SendEvent(
                CS_BASELINE_CFECORE_INF_EID, CFE_EVS_EventType_INFORMATION,
...
void CS_ReportBaselineOSCmd(CFE_SB_MsgPtr_t MessagePtr) {
    ...
    if (CS_VerifyCmdLength(MessagePtr, ExpectedLength)) {
        if (CS_AppData.OSCodeSeg.ComputedYet == true) {
            CFE_EVS_SendEvent(
                CS_BASELINE_OS_INF_EID, CFE_EVS_EventType_INFORMATION,
                "Baseline of OS code segment is 0x%08X",
                (unsigned int)CS_AppData.OSCodeSeg.ComparisonValue);
        } else {
            CFE_EVS_SendEvent(
                CS_NO_BASELINE_OS_INF_EID, CFE_EVS_EventType_INFORMATION,
...
void CS_RecomputeBaselineCfeCoreCmd(CFE_SB_MsgPtr_t MessagePtr) {
    ...
    /* Verify command packet length... */
    if (CS_VerifyCmdLength(MessagePtr, ExpectedLength)) {
        if (CS_AppData.RecomputeInProgress == false &&
            ...
            Status = CFE_ES_CreateChildTask(
                &ChildTaskID, CS_RECOMP_CFECORE_TASK_NAME,
                CS_RecomputeEepromMemoryChildTask, NULL,
            ...
            } else /* child task creation failed */
            {
                CFE_EVS_SendEvent(CS_RECOMPUTE_CFECORE_CREATE_CHDTASK_ERR_EID,
                                  ...
                                  "CFE_ES_CreateChildTask returned: 0x%08X",
                                  (unsigned int)Status);
...
void CS_RecomputeBaselineOSCmd(CFE_SB_MsgPtr_t MessagePtr) {
    ...
    if (CS_VerifyCmdLength(MessagePtr, ExpectedLength)) {
        if (CS_AppData.RecomputeInProgress == false &&
            ...
            Status = CFE_ES_CreateChildTask(
                &ChildTaskID, CS_RECOMP_OS_TASK_NAME,
                CS_RecomputeEepromMemoryChildTask, NULL,
                CFE_PLATFORM_ES_DEFAULT_STACK_SIZE, CS_CHILD_TASK_PRIORITY, 0);
            if (Status == CFE_SUCCESS) {
            ...
            /*send event that we can't start another task right now */
            CFE_EVS_SendEvent(
                CS_RECOMPUTE_OS_CHDTASK_ERR_EID, CFE_EVS_EventType_ERROR,
...
void CS_OneShotCmd(CFE_SB_MsgPtr_t MessagePtr) {
    ...
    if (CS_VerifyCmdLength(MessagePtr, ExpectedLength)) {
        ...
        Status = CFE_PSP_MemValidateRange(CmdPtr->Address, CmdPtr->Size,
                                          CFE_PSP_MEM_ANY);
        ...
        if (Status == OS_SUCCESS) {
            if (CS_AppData.RecomputeInProgress == false &&
                ...
                if (CmdPtr->MaxBytesPerCycle == 0) {
                    ...
                    CS_AppData.LastOneShotMaxBytesPerCycle =
                        CmdPtr->MaxBytesPerCycle;
                }

                CS_AppData.LastOneShotChecksum = 0;

                Status = CFE_ES_CreateChildTask(
                    &ChildTaskID, CS_ONESHOT_TASK_NAME, CS_OneShotChildTask,
                    NULL, CFE_PLATFORM_ES_DEFAULT_STACK_SIZE,
                    CS_CHILD_TASK_PRIORITY, 0);
                if (Status == CFE_SUCCESS) {
                    CFE_EVS_SendEvent(
                        CS_ONESHOT_STARTED_DBG_EID, CFE_EVS_EventType_DEBUG,
    ...
    if (CS_VerifyCmdLength(MessagePtr, ExpectedLength)) {
        ...
        if (CS_AppData.RecomputeInProgress == false &&
            ...
            if (Status == CFE_SUCCESS) {
                ...
                CS_AppData.CmdCounter++;
                CFE_EVS_SendEvent(
                    CS_ONESHOT_CANCELLED_INF_EID, CFE_EVS_EventType_INFORMATION,
                    "OneShot checksum calculation has been cancelled");
            } else {
                CFE_EVS_SendEvent(CS_ONESHOT_CANCEL_DELETE_CHDTASK_ERR_EID,
                                  CFE_EVS_EventType_ERROR,
                                  "Cancel OneShot checksum failed, "
                                  "CFE_ES_DeleteChildTask returned:  0x%08X",
                                  (unsigned int)Status);
                CS_AppData.CmdErrCounter++;
            }
        } else {
            CFE_EVS_SendEvent(
                CS_ONESHOT_CANCEL_NO_CHDTASK_ERR_EID, CFE_EVS_EventType_ERROR,
