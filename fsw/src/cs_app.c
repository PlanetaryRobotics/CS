...
#define CS_NUM_DATA_STORE_STATES \
    6 /* 4 tables + OS CS + cFE core number of checksum states for CDS */
...
void CS_AppMain(void) {
    ...
    while (CFE_ES_RunLoop(&CS_AppData.RunStatus)) {
        ...
        Result = CFE_SB_RcvMsg(&CS_AppData.MsgPtr, CS_AppData.CmdPipe,
                               CFE_SB_PEND_FOREVER);
    ...
    if (CS_AppData.RunStatus == CFE_ES_RunStatus_APP_ERROR ||
        ...
        CFE_EVS_SendEvent(CS_EXIT_ERR_EID, CFE_EVS_EventType_ERROR,
         ...
         */
        CFE_EVS_SendEvent(CS_EXIT_INF_EID, CFE_EVS_EventType_INFORMATION,
...
int32 CS_AppInit(void) {
    ...
    CFE_SB_InitMsg(&CS_AppData.HkPacket, CS_HK_TLM_MID, sizeof(CS_HkPacket_t),
                   true);

    /* Create Software Bus message pipe */

    Result = CFE_SB_CreatePipe(&CS_AppData.CmdPipe, CS_AppData.PipeDepth,
                               CS_AppData.PipeName);
    if (Result != CFE_SUCCESS) {
        CFE_EVS_SendEvent(
            CS_INIT_SB_CREATE_ERR_EID, CFE_EVS_EventType_ERROR,
    ...
    Result = CFE_SB_Subscribe(CS_SEND_HK_MID, CS_AppData.CmdPipe);

    if (Result != CFE_SUCCESS) {
        CFE_EVS_SendEvent(
            CS_INIT_SB_SUBSCRIBE_HK_ERR_EID, CFE_EVS_EventType_ERROR,
    ...
    Result = CFE_SB_Subscribe(CS_BACKGROUND_CYCLE_MID, CS_AppData.CmdPipe);

    if (Result != CFE_SUCCESS) {
        CFE_EVS_SendEvent(
            CS_INIT_SB_SUBSCRIBE_BACK_ERR_EID, CFE_EVS_EventType_ERROR,
    ...
    Result = CFE_SB_Subscribe(CS_CMD_MID, CS_AppData.CmdPipe);
    if (Result != CFE_SUCCESS) {
    ...
    ResultInit = CS_TableInit(
        ...
        (void *)&CS_AppData.ResEepromTblPtr, CS_EEPROM_TABLE,
        CS_DEF_EEPROM_TABLE_NAME, CS_RESULTS_EEPROM_TABLE_NAME,
    ...
    ResultInit = CS_TableInit(
        &CS_AppData.DefMemoryTableHandle, &CS_AppData.ResMemoryTableHandle,
        (void *)&CS_AppData.DefMemoryTblPtr,
        (void *)&CS_AppData.ResMemoryTblPtr, CS_MEMORY_TABLE,
        CS_DEF_MEMORY_TABLE_NAME, CS_RESULTS_MEMORY_TABLE_NAME,
    ...
    ResultInit = CS_TableInit(
        ...
        (void *)&CS_AppData.DefAppTblPtr, (void *)&CS_AppData.ResAppTblPtr,
        CS_APP_TABLE, CS_DEF_APP_TABLE_NAME, CS_RESULTS_APP_TABLE_NAME,
        CS_MAX_NUM_APP_TABLE_ENTRIES, CS_DEF_APP_TABLE_FILENAME,
        &CS_AppData.DefaultAppDefTable, sizeof(CS_Def_App_Table_Entry_t),
    ...
    ResultInit = CS_TableInit(
        ...
        (void *)&CS_AppData.ResTablesTblPtr, CS_TABLES_TABLE,
        CS_DEF_TABLES_TABLE_NAME, CS_RESULTS_TABLES_TABLE_NAME,
    ...
    ResultSegment =
        CFE_PSP_GetKernelTextSegmentInfo(&KernelAddress, &KernelSize);
    ...
    if (Result == CFE_SUCCESS) {
        Result = CFE_EVS_SendEvent(
            CS_INIT_INF_EID, CFE_EVS_EventType_INFORMATION,
            "CS Initialized. Version %d.%d.%d.%d", CS_MAJOR_VERSION,
...
int32 CS_AppPipe(CFE_SB_MsgPtr_t MessagePtr) {
    ...
    switch (MessageID) {
        ...
        case CS_SEND_HK_MID:
            ...
            /* update each table if there is no recompute happening on that
             * table */
            ...
            if (!((CS_AppData.RecomputeInProgress == true) &&
                ...
                Result = CS_HandleTableUpdate(
                    ...
                    CS_AppData.ResEepromTableHandle, CS_EEPROM_TABLE,
                    CS_MAX_NUM_EEPROM_TABLE_ENTRIES);

                if (Result != CFE_SUCCESS) {
                    CS_AppData.EepromCSState = CS_STATE_DISABLED;
                    Result                   = CFE_EVS_SendEvent(
                        CS_UPDATE_EEPROM_ERR_EID, CFE_EVS_EventType_ERROR,
                        "Table update failed for Eeprom: 0x%08X, checksumming "
                        "Eeprom is disabled",
                        (unsigned int)Result);
                }
            }
            ...
            if (!((CS_AppData.RecomputeInProgress == true) &&
                ...
                Result = CS_HandleTableUpdate(
                    ...
                    CS_AppData.ResMemoryTableHandle, CS_MEMORY_TABLE,
                    CS_MAX_NUM_MEMORY_TABLE_ENTRIES);
                if (Result != CFE_SUCCESS) {
                    CS_AppData.MemoryCSState = CS_STATE_DISABLED;
                    Result                   = CFE_EVS_SendEvent(
                        CS_UPDATE_MEMORY_ERR_EID, CFE_EVS_EventType_ERROR,
                        "Table update failed for Memory: 0x%08X, checksumming "
                        "Memory is disabled",
                        (unsigned int)Result);
                }
            }
            ...
            if (!((CS_AppData.RecomputeInProgress == true) &&
                ...
                if (Result != CFE_SUCCESS) {
                    CS_AppData.AppCSState = CS_STATE_DISABLED;
                    Result                = CFE_EVS_SendEvent(
                        CS_UPDATE_APP_ERR_EID, CFE_EVS_EventType_ERROR,
                        "Table update failed for Apps: 0x%08X, checksumming "
                        "Apps is disabled",
                        (unsigned int)Result);
                }
            }
            ...
            if (!((CS_AppData.RecomputeInProgress == true) &&
                ...
                Result = CS_HandleTableUpdate(
                    ...
                    CS_AppData.ResTablesTableHandle, CS_TABLES_TABLE,
                    CS_MAX_NUM_TABLES_TABLE_ENTRIES);

                if (Result != CFE_SUCCESS) {
                    CS_AppData.TablesCSState = CS_STATE_DISABLED;
                    Result                   = CFE_EVS_SendEvent(
                        CS_UPDATE_TABLES_ERR_EID, CFE_EVS_EventType_ERROR,
                        "Table update failed for Tables: 0x%08X, checksumming "
                        "Tables is disabled",
                        (unsigned int)Result);
                }
            }
        ...
        case CS_CMD_MID:
            ...
            switch (CommandCode) {
                ...
                default:
                    CFE_EVS_SendEvent(
                        CS_CC1_ERR_EID, CFE_EVS_EventType_ERROR,
...
void CS_HousekeepingCmd(CFE_SB_MsgPtr_t MessagePtr) {
    ...
    if (ExpectedLength != ActualLength) {
        ...
        CFE_EVS_SendEvent(
            CS_LEN_ERR_EID, CFE_EVS_EventType_ERROR,
        ...
        CS_AppData.HkPacket.RecomputeInProgress =
            (uint8)CS_AppData.RecomputeInProgress;
        CS_AppData.HkPacket.OneShotInProgress =
            (uint8)CS_AppData.OneShotInProgress;
        ...
        CS_AppData.HkPacket.LastOneShotMaxBytesPerCycle =
...
int32 CS_CreateRestoreStatesFromCDS(void) {
    ...
    */
    ...
    if (Result == CFE_SUCCESS) {
        ...
        */
        Result =
            CFE_ES_RestoreFromCDS(DataStoreBuffer, CS_AppData.DataStoreHandle);
