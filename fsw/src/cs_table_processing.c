...
int32 CS_ValidateEepromChecksumDefinitionTable(void *TblPtr) {
    ...
    for (OuterLoop = 0; OuterLoop < CS_MAX_NUM_EEPROM_TABLE_ENTRIES;
         OuterLoop++) {
        ...
        if ((StateField == CS_STATE_EMPTY) ||
            ...
            /* If the StateField is within this range, it's check if it's not
             * empty. */
            if (StateField == CS_STATE_DISABLED ||
                StateField == CS_STATE_ENABLED) {
                Status =
                    CFE_PSP_MemValidateRange(Address, Size, CFE_PSP_MEM_EEPROM);
                if (Status != OS_SUCCESS) {
                    ...
                    if (Result != CS_TABLE_ERROR) {
                        CFE_EVS_SendEvent(
                            ...
                            "Eeprom Table Validate: Illegal checksum range "
                            "found in Entry ID %d, CFE_PSP_MemValidateRange "
                            "returned: 0x%08X",
    ...
    CFE_EVS_SendEvent(CS_VAL_EEPROM_INF_EID, CFE_EVS_EventType_INFORMATION,
                      "CS Eeprom Table verification results: good = %d, bad = "
                      "%d, unused = %d",
                      (int)GoodCount, (int)BadCount, (int)EmptyCount);
...
int32 CS_ValidateMemoryChecksumDefinitionTable(void *TblPtr) {
    ...
    for (OuterLoop = 0; OuterLoop < CS_MAX_NUM_MEMORY_TABLE_ENTRIES;
         OuterLoop++) {
        OuterEntry = &StartOfTable[OuterLoop];
        ...
        if ((StateField == CS_STATE_EMPTY) ||
            ...
            if (StateField == CS_STATE_DISABLED ||
                StateField == CS_STATE_ENABLED) {
                Status =
                    CFE_PSP_MemValidateRange(Address, Size, CFE_PSP_MEM_ANY);
                if (Status != OS_SUCCESS) {
                    BadCount++;
                    if (Result != CS_TABLE_ERROR) {
                        CFE_EVS_SendEvent(
                            ...
                            CFE_EVS_EventType_ERROR,
                            "Memory Table Validate: Illegal checksum range "
                            "found in Entry ID %d, CFE_PSP_MemValidateRange "
                            "returned: 0x%08X",
                            (int)OuterLoop, (unsigned int)Status);
                        Result = CS_TABLE_ERROR;
            ...
            BadCount++;
            if (Result != CS_TABLE_ERROR) {
                CFE_EVS_SendEvent(CS_VAL_MEMORY_STATE_ERR_EID,
                                  CFE_EVS_EventType_ERROR,
    ...
    CFE_EVS_SendEvent(CS_VAL_MEMORY_INF_EID, CFE_EVS_EventType_INFORMATION,
                      "CS Memory Table verification results: good = %d, bad = "
                      "%d, unused = %d",
                      (int)GoodCount, (int)BadCount, (int)EmptyCount);
...
int32 CS_ValidateTablesChecksumDefinitionTable(void *TblPtr) {
    ...
    for (OuterLoop = 0; OuterLoop < CS_MAX_NUM_TABLES_TABLE_ENTRIES;
         OuterLoop++) {
        OuterEntry = &StartOfTable[OuterLoop];
        ...
        if (strlen(OuterEntry->Name) != 0) {
            ...
            if (((StateField == CS_STATE_EMPTY) ||
                ...
                for (InnerLoop = OuterLoop + 1;
                     InnerLoop < CS_MAX_NUM_TABLES_TABLE_ENTRIES; InnerLoop++) {
                    if (strncmp(OuterEntry->Name,
                                (&StartOfTable[InnerLoop])->Name,
                                CFE_TBL_MAX_FULL_NAME_LEN) == 0) {
                        ...
                        if (Result != CS_TABLE_ERROR) {
                            ...
                            CFE_EVS_SendEvent(
                                ...
                                CFE_EVS_EventType_ERROR,
                                "CS Tables Table Validate: Duplicate Name (%s) "
                                "found at entries %d and %d",
                                OuterEntry->Name, (int)InnerLoop,
                                (int)OuterLoop);
                ...
                if (Result != CS_TABLE_ERROR) {
                    CFE_EVS_SendEvent(
                        CS_VAL_TABLES_STATE_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CS Tables Table Validate: Illegal State Field "
                        "(0x%04X) found with name %s",
            ...
            if (StateField != CS_STATE_EMPTY) {
                ...
                if (Result != CS_TABLE_ERROR) {
                    CFE_EVS_SendEvent(CS_VAL_TABLES_DEF_TBL_ZERO_NAME_ERR_EID,
                                      CFE_EVS_EventType_ERROR,
                                      "CS Tables Table Validate: Illegal State "
                                      "(0x%04X) with empty name at entry %d",
    ...
    CFE_EVS_SendEvent(CS_VAL_TABLES_INF_EID, CFE_EVS_EventType_INFORMATION,
                      "CS Tables Table verification results: good = %d, bad = "
                      "%d, unused = %d",
                      (int)GoodCount, (int)BadCount, (int)EmptyCount);
...
int32 CS_ValidateAppChecksumDefinitionTable(void *TblPtr) {
    ...
    for (OuterLoop = 0; OuterLoop < CS_MAX_NUM_APP_TABLE_ENTRIES; OuterLoop++) {
        ...
        if (strlen(OuterEntry->Name) != 0) {
            ...
            if (((StateField == CS_STATE_EMPTY) ||
                ...
                /* Verify the name field is not duplicated */
                for (InnerLoop = OuterLoop + 1;
                     InnerLoop < CS_MAX_NUM_APP_TABLE_ENTRIES; InnerLoop++) {
                    if (strncmp(OuterEntry->Name,
                                (&StartOfTable[InnerLoop])->Name,
                                CFE_TBL_MAX_FULL_NAME_LEN) == 0) {
                        if (DuplicateFound != true) {
                        ...
                        if (Result != CS_TABLE_ERROR) {
                            ...
                            CFE_EVS_SendEvent(
                                ...
                                CFE_EVS_EventType_ERROR,
                                "CS Apps Table Validate: Duplicate Name (%s) "
                                "found at entries %d and %d",
                                OuterEntry->Name, (int)InnerLoop,
                                (int)OuterLoop);
                ...
                if (Result != CS_TABLE_ERROR) {
                    CFE_EVS_SendEvent(
                        CS_VAL_APP_STATE_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CS Apps Table Validate: Illegal State Field (0x%04X) "
                        "found with name %s",
                        (unsigned short)StateField, OuterEntry->Name);
    ...
    } /* for (OuterLoop = 0; OuterLoop < CS_MAX_NUM_APPS_TABLE_ENTRIES;
         OuterLoop++) */

    CFE_EVS_SendEvent(
        CS_VAL_APP_INF_EID, CFE_EVS_EventType_INFORMATION,
...
/*                                                                 */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
void CS_ProcessNewEepromMemoryDefinitionTable(
    CS_Def_EepromMemory_Table_Entry_t *DefinitionTblPtr,
    ...
    CFE_PSP_MemCpy(&StartOfResultsTable, ResultsTblPtr,
                   sizeof(StartOfResultsTable));
    CFE_PSP_MemCpy(&StartOfDefTable, DefinitionTblPtr, sizeof(StartOfDefTable));

    strncpy(&TableType[0], "Undef Tbl",
            CS_TABLETYPE_NAME_SIZE); /* Init the table type string */
    ...
    for (Loop = 0; Loop < NumEntries; Loop++) {
        ...
        if (DefEntry->State != CS_STATE_EMPTY) {
            /* This is just a real simple test, because all of the exception
             handling has already been done by the Validation routine above */
            NumRegionsInTable++;
...
/*                                                                 */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
void CS_ProcessNewTablesDefinitionTable(
    CS_Def_Tables_Table_Entry_t *DefinitionTblPtr,
    ...
    for (Loop = 0; Loop < CS_MAX_NUM_TABLES_TABLE_ENTRIES; Loop++) {
        ...
        if (DefEntry->State != CS_STATE_EMPTY) {
            ...
            /* move entry index to the dot, string terminator, or end of table
             * entry */
            ...
            ResultsEntry->NumBytesToChecksum =
                0; /* this is unknown at this time */
            ResultsEntry->ComparisonValue   = 0;
            ResultsEntry->ByteOffset        = 0;
            ResultsEntry->TempChecksumValue = 0;
            ...
            strncpy(ResultsEntry->Name, DefEntry->Name,
...
int32 CS_TableInit(CFE_TBL_Handle_t *DefinitionTableHandle,
                   CFE_TBL_Handle_t *ResultsTableHandle, void *DefinitionTblPtr,
                   void *ResultsTblPtr, uint16 Table,
                   const char *DefinitionTableName,
                   const char *ResultsTableName, uint16 NumEntries,
    ...
    strncpy(TableType, "Undef Tbl",
            CS_TABLETYPE_NAME_SIZE); /* Init table type */

    SizeOfTable = NumEntries * SizeofResultsTableEntry;

    Result =
        CFE_TBL_Register(ResultsTableHandle, ResultsTableName, SizeOfTable,
    ...
    if (Result == CFE_SUCCESS) {
        ...
        Result = CFE_TBL_Register(
            DefinitionTableHandle, DefinitionTableName, SizeOfTable,
    ...
    if ((Result == CFE_SUCCESS) && (ValidFile == true)) {
        Result         = CFE_TBL_Load(*DefinitionTableHandle, CFE_TBL_SRC_FILE,
    ...
    if (ResultFromLoad != CFE_SUCCESS) {
        Result = CFE_TBL_Load(*DefinitionTableHandle, CFE_TBL_SRC_ADDRESS,
        ...
        if ((Result == CFE_TBL_INFO_UPDATED)) {
            if (Table == CS_APP_TABLE) {
                CS_ProcessNewAppDefinitionTable(
                    (CS_Def_App_Table_Entry_t *)DefinitionTblPtr,
                    (CS_Res_App_Table_Entry_t *)ResultsTblPtr);
            } else {
    ...
    if (Result >= CFE_SUCCESS) {
        ...
        CFE_EVS_SendEvent(
            CS_TBL_INIT_ERR_EID, CFE_EVS_EventType_ERROR,
            "CS received error 0x%08X initializing Definition table for %s",
            (unsigned int)Result, TableType);
    }

    /* If we loaded from file successfully then the states we wish to use have
     * already been set If we loaded from memory then disable the table  */
...
/*                                                                 */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
int32 CS_HandleTableUpdate(void *DefinitionTblPtr, void *ResultsTblPtr,
                           CFE_TBL_Handle_t DefinitionTableHandle,
                           CFE_TBL_Handle_t ResultsTableHandle, uint16 Table,
                           uint16 NumEntries) {
    ...
    /* Below, there are several values that are returned and assigned, but never
     * evaluated. */
    /* This is done so intentionally, as it helps us with Source-Level debugging
     * this functions. */
    ...
    if (Result == CFE_SUCCESS) {
        GetResult2 =
            CFE_TBL_GetAddress(DefinitionTblPtr, DefinitionTableHandle);
        Result = GetResult2;
    }
    ...
    if ((Result == CFE_TBL_INFO_UPDATED)) {
        if (Table == CS_TABLES_TABLE) {
            ...
            for (Loop = 0; Loop < CS_MAX_NUM_TABLES_TABLE_ENTRIES; Loop++) {
                if (CS_AppData.ResTablesTblPtr[Loop].TblHandle !=
                    CFE_TBL_BAD_TABLE_HANDLE) {
                    if (CS_AppData.ResTablesTblPtr[Loop].IsCSOwner == false) {
                        CFE_TBL_Unregister(
                            CS_AppData.ResTablesTblPtr[Loop].TblHandle);
                    }
                }
            }
        ...
        if (Result < CFE_SUCCESS) {
            ...
            CFE_EVS_SendEvent(
                CS_TBL_UPDATE_ERR_EID, CFE_EVS_EventType_ERROR,
                "CS had problems updating table. Res Release: 0x%08X Def "
                "Release:0x%08X Res Manage:0x%08X Def Manage: 0x%08X "
                "Get:0x%08X for table %s",
                (unsigned int)ReleaseResult1, (unsigned int)ReleaseResult2,
                (unsigned int)ManageResult1, (unsigned int)ManageResult2,
                (unsigned int)GetResult2, TableType);
        }
    }
    return (Result);
