    ...
    if (CS_VerifyCmdLength(MessagePtr, ExpectedLength)) {
        ...
        if ((EntryID < CS_MAX_NUM_EEPROM_TABLE_ENTRIES) &&
            ...
            CFE_EVS_SendEvent(CS_BASELINE_INVALID_ENTRY_EEPROM_ERR_EID,
                              ...
                              "Eeprom report baseline failed, Entry ID "
                              "invalid: %d, State: %d Max ID: %d",
                              EntryID, State,
        ...
        if (CS_AppData.RecomputeInProgress == false &&
            ...
            if ((EntryID < CS_MAX_NUM_EEPROM_TABLE_ENTRIES) &&
                ...
                CS_AppData.RecomputeEepromMemoryEntryPtr =
                    &CS_AppData.ResEepromTblPtr[EntryID];

                Status = CFE_ES_CreateChildTask(
                    &ChildTaskID, CS_RECOMP_EEPROM_TASK_NAME,
                    CS_RecomputeEepromMemoryChildTask, NULL,
                    CFE_PLATFORM_ES_DEFAULT_STACK_SIZE, CS_CHILD_TASK_PRIORITY,
                    0);
                ...
                } else /* child task creation failed */
                {
                    CFE_EVS_SendEvent(
                        ...
                        CFE_EVS_EventType_ERROR,
                        "Recompute baseline of Eeprom Entry ID %d failed, "
                ...
                CFE_EVS_SendEvent(CS_RECOMPUTE_INVALID_ENTRY_EEPROM_ERR_EID,
                                  CFE_EVS_EventType_ERROR,
                                  "Eeprom recompute baseline of entry failed, "
                                  "Entry ID invalid: %d, State: %d, Max ID: %d",
                                  EntryID, State,
                                  (CS_MAX_NUM_EEPROM_TABLE_ENTRIES - 1));
            ...
            CFE_EVS_SendEvent(CS_RECOMPUTE_EEPROM_CHDTASK_ERR_EID,
                              CFE_EVS_EventType_ERROR,
                              "Recompute baseline of Eeprom Entry ID %d "
                              "failed: child task in use",
                              EntryID);
            CS_AppData.CmdErrCounter++;
        }
    }
...
void CS_EnableEntryIDEepromCmd(CFE_SB_MsgPtr_t MessagePtr) {
    ...
    if (CS_VerifyCmdLength(MessagePtr, ExpectedLength)) {
        ...
        if ((EntryID < CS_MAX_NUM_EEPROM_TABLE_ENTRIES) &&
            ...
            CFE_EVS_SendEvent(
                CS_ENABLE_EEPROM_ENTRY_INF_EID, CFE_EVS_EventType_INFORMATION,
            ...
            if (CS_AppData.DefEepromTblPtr[EntryID].State != CS_STATE_EMPTY) {
                ...
                CFE_EVS_SendEvent(CS_ENABLE_EEPROM_DEF_EMPTY_DBG_EID,
                                  CFE_EVS_EventType_DEBUG,
                                  "CS unable to update Eeprom definition table "
                                  "for entry %d, State: %d",
                                  EntryID, State);
            }
            ...
            CFE_EVS_SendEvent(
                CS_ENABLE_EEPROM_INVALID_ENTRY_ERR_EID, CFE_EVS_EventType_ERROR,
                "Enable Eeprom entry failed, invalid Entry ID:  %d, State: %d, "
                "Max ID: %d",
...
void CS_DisableEntryIDEepromCmd(CFE_SB_MsgPtr_t MessagePtr) {
    ...
    if (CS_VerifyCmdLength(MessagePtr, ExpectedLength)) {
        ...
        if ((EntryID < CS_MAX_NUM_EEPROM_TABLE_ENTRIES) &&
            ...
            CFE_EVS_SendEvent(
                CS_DISABLE_EEPROM_ENTRY_INF_EID, CFE_EVS_EventType_INFORMATION,
            ...
            CFE_EVS_SendEvent(CS_DISABLE_EEPROM_INVALID_ENTRY_ERR_EID,
                              CFE_EVS_EventType_ERROR,
                              "Disable Eeprom entry failed, invalid Entry ID:  "
                              "%d, State: %d, Max ID: %d",
                              EntryID, State,
                              (CS_MAX_NUM_EEPROM_TABLE_ENTRIES - 1));
    ...
    if (CS_VerifyCmdLength(MessagePtr, ExpectedLength)) {
        ...
        for (Loop = 0; Loop < CS_MAX_NUM_EEPROM_TABLE_ENTRIES; Loop++) {
            ...
            if ((ResultsEntry.StartAddress <= CmdPtr->Address) &&
                CmdPtr->Address <= (ResultsEntry.StartAddress +
                ...
                CFE_EVS_SendEvent(CS_GET_ENTRY_ID_EEPROM_INF_EID,
                                  ...
                                  (unsigned int)(CmdPtr->Address), Loop);
                EntryFound = true;
            }
        }
