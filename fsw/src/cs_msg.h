...
typedef struct {
    ...
    uint16 CurrentEntryInTable; /**< \cstlmmnemonic \CS_CURRENTRYINTABLE
                                     \brief Current entry ID in the table being
                                   checksummed */
    ...
    uint32 LastOneShotAddress;          /**< \cstlmmnemonic \CS_LASTONESHOTADDR
                                             \brief Address used in last one shot
                                           checksum command */
    uint32 LastOneShotSize;             /**< \cstlmmnemonic \CS_LASTONESHOTSIZE
                                             \brief Size used in the last one shot checksum
                                           command */
    uint32 LastOneShotMaxBytesPerCycle; /**< \cstlmmnemonic
                                           \CS_LASTONESHOTMAXBYTESPERCYCLE
                                             \brief Max bytes per cycle for last
                                           one shot checksum command */
    uint32 LastOneShotChecksum; /**< \cstlmmnemonic \CS_LASTONESHOTCHECKSUM
                                     \brief Checksum of the last one shot
                                   checksum command */

    uint32 PassCounter; /**< \cstlmmnemonic \CS_PASSCOUNTER
                             \brief Number of times CS has passed through all of
                           its tables */
} CS_HkPacket_t;
 ...
 *#CS_DISABLE_ALL_CS_CC,
 **  #CS_ENABLE_CFECORE_CC, #CS_DISABLE_CFECORE_CC, #CS_ENABLE_OS_CC,
 *#CS_DISABLE_OS_CC, #CS_ENABLE_EEPROM_CC,
 **  #CS_DISABLE_EEPROM_CC, #CS_ENABLE_MEMORY_CC, #CS_DISABLE_MEMORY_CC,
 *#CS_ENABLE_TABLES_CC, #CS_DISABLE_TABLES_CC
 ...
 **  For command details see CS_GET_ENTRY_ID_EEPROM_CC,
 ...
 **  For command details see #CS_ENABLE_ENTRY_EEPROM_CC,
 *#CS_DISABLE_ENTRY_EEPROM_CC,
 ...
 **  For command details see #CS_ENABLE_NAME_TABLE_CC,
 *#CS_DISABLE_NAME_TABLE_CC,
 **  #CS_RECOMPUTE_BASELINE_TABLE_CC, #CS_REPORT_BASELINE_TABLE_CC
 */
typedef struct {
    CFE_SB_CmdHdr_t CmdHeader;
    char Name[CFE_TBL_MAX_FULL_NAME_LEN]; /**< \brief Table name to perform a
                                             command on */
} CS_TableNameCmd_t;
...
typedef struct {
    ...
    uint32
        MaxBytesPerCycle; /**< \brief Max Number of bytes to compute per cycle.
                             Value of Zero to use platform config value */
} CS_OneShotCmd_t;
