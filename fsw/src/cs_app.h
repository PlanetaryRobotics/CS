...
/**\{ */
#define CS_SUCCESS                      \
    0 /**< \brief Success return code   \
            when a checksum compare did \
            not fail*/
#define CS_ERROR                         \
    (-1) /**< \brief Error code returned \
               when a checksum compare   \
               failed*/
#define CS_ERR_NOT_FOUND                        \
    (-2) /**< \brief Error code returned        \
               on a Table or App operation      \
               where the app or table requested \
               could not be found */
#define CS_TABLE_ERROR                   \
    (-3) /**< \brief Error code returned \
...
typedef struct {
    ...
    uint16 ChildTaskEntryID;  /**< \brief Entry in table for child task to
                                 process */
    bool RecomputeInProgress; /**< \brief Flag for a recompute in progress */
    bool OneShotInProgress;   /**< \brief Flag for a one shot calculation in
                                 progress*/
    uint32 ChildTaskID;       /**< \brief Task ID for the child task*/

    uint32 MaxBytesPerCycle; /**< \brief Max number of bytes to process in a
                                cycle*/

    uint32 RunStatus; /**< \brief Application run status */

    CS_Res_EepromMemory_Table_Entry_t*
        RecomputeEepromMemoryEntryPtr; /**< \brief Pointer to an entry to
                                          recompute in the Eeprom or Memory
                                          table*/

    CS_Res_App_Table_Entry_t*
        RecomputeAppEntryPtr; /**< \brief Pointer to an entry to recompute in
                                 the Application table*/
    CS_Res_Tables_Table_Entry_t*
        RecomputeTablesEntryPtr; /**< \brief Pointer to an entry to recompute in
                                    the Tables table*/
    ...
    CFE_TBL_Handle_t DefEepromTableHandle; /**< \brief Handle to the Eeprom
                                              definition table*/
    CFE_TBL_Handle_t
        ResEepromTableHandle; /**< \brief Handle to the Eeprom results table*/

    CFE_TBL_Handle_t DefMemoryTableHandle; /**< \brief Handle to the Memory
                                              definition table*/
    CFE_TBL_Handle_t
        ResMemoryTableHandle; /**< \brief Handle to the Memory results table*/

    CFE_TBL_Handle_t DefTablesTableHandle; /**< \brief Handle to the Tables
                                              definition table*/
    CFE_TBL_Handle_t
        ResTablesTableHandle; /**< \brief Handle to the Tables results table*/

    CFE_TBL_Handle_t
        DefAppTableHandle; /**< \brief Handle to the Apps definition table*/
    CFE_TBL_Handle_t
        ResAppTableHandle; /**< \brief Hanlde to the Apps results table */

    CS_Def_EepromMemory_Table_Entry_t*
        DefEepromTblPtr; /**< \brief Pointer to the Eeprom definition table */
    CS_Res_EepromMemory_Table_Entry_t*
        ResEepromTblPtr; /**< \brief Pointer to the Eeprom results table */

    CS_Def_EepromMemory_Table_Entry_t*
        DefMemoryTblPtr; /**< \brief Pointer to the Memory definition table */
    CS_Res_EepromMemory_Table_Entry_t*
        ResMemoryTblPtr; /**< \brief Pointer to the Memory results table */

    CS_Def_Tables_Table_Entry_t*
        DefTablesTblPtr; /**< \brief Pointer to the Tables definition table */
    CS_Res_Tables_Table_Entry_t*
        ResTablesTblPtr; /**< \brief Pointer to the Tables results table */

    CS_Def_App_Table_Entry_t*
        DefAppTblPtr; /**< \brief Pointer to the Apps definition table */
    CS_Res_App_Table_Entry_t*
        ResAppTblPtr; /**< \brief Pointer to the Apps results table */

    CS_Res_EepromMemory_Table_Entry_t
        OSCodeSeg; /**< \brief OS code segment 'table' */
    CS_Res_EepromMemory_Table_Entry_t
        CfeCoreCodeSeg; /**< \brief cFE core code segment 'table' */

    CS_Def_EepromMemory_Table_Entry_t DefaultEepromDefTable
        [CS_MAX_NUM_EEPROM_TABLE_ENTRIES]; /**< \brief Default Eeprom definition
                                              table */
    CS_Def_EepromMemory_Table_Entry_t DefaultMemoryDefTable
        [CS_MAX_NUM_MEMORY_TABLE_ENTRIES]; /**< \brief Default Memory definition
                                              table */
    CS_Def_Tables_Table_Entry_t DefaultTablesDefTable
        [CS_MAX_NUM_TABLES_TABLE_ENTRIES]; /**< \brief Default Tables definition
                                              table */
    CS_Def_App_Table_Entry_t
        DefaultAppDefTable[CS_MAX_NUM_APP_TABLE_ENTRIES]; /**< \brief Default
                                                             Apps definition
                                                             table */

    /* The following pointers locate the results for CS tables that get
     * checksummed because they are listed in the CS Tables table */

    CS_Res_Tables_Table_Entry_t*
        EepResTablesTblPtr; /**< \brief Pointer to CS Tables table results entry
                               for the CS eeprom table */
    CS_Res_Tables_Table_Entry_t*
        MemResTablesTblPtr; /**< \brief Pointer to CS Tables table results entry
                               for the CS memory table */
    CS_Res_Tables_Table_Entry_t*
        AppResTablesTblPtr; /**< \brief Pointer to CS Tables table results entry
                               for the CS apps table */
    CS_Res_Tables_Table_Entry_t*
        TblResTablesTblPtr; /**< \brief Pointer to CS Tables table results entry
                               for the CS Tables table */

#if (CS_PRESERVE_STATES_ON_PROCESSOR_RESET == TRUE)
    CFE_ES_CDSHandle_t DataStoreHandle; /**< \brief Handle to critical data
                                           store created by CS */
#endif
    ...
    uint16 CurrentEntryInTable; /**< \brief Current entry ID in the table being
                                   checksummed */
    ...
    uint32 LastOneShotAddress; /**< \brief Address used in last one shot
                                  checksum command */
    uint32 LastOneShotSize; /**< \brief Size used in the last one shot checksum
                               command */
    uint32 LastOneShotMaxBytesPerCycle; /**< \brief Maximum bytes to process
                                           each cycle during last one shot
                                           checksum command */
    uint32 LastOneShotChecksum; /**< \brief Checksum of the last one shot
                                   checksum command */

    uint32 PassCounter; /**< \brief Number of times CS has passed through all of
                           its tables */
